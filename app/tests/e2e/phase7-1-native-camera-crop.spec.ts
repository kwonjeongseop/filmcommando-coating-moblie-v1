...
test.describe('Phase 7-1: 자체 카메라 촬영 후 자르기 리뷰', () => {
  ...
  test('촬영 즉시 EditorScreen이 아니라 자르기 리뷰 화면이 먼저 뜬다', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: '원본으로 편집기 이동' })).toBeVisible();
    await expect(page.getByRole('button', { name: '스캔으로 편집기 이동' })).toBeVisible();
  ...
  test('원본으로 편집기 이동을 누르면 EditorScreen으로 진입하고 문서 영역만 크롭된다', async ({ page }) => {
    await openCameraAndCapture(page);
